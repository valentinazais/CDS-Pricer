...
(function () {
    ...
    function run() {
        ...
        // calibrate hazard rate from MARKET spread (not contractual)
        var h = CDS.calibrateHazardRate(p.marketSpreadBps, p.recoveryRate, p.riskFreeRate, p.maturity, p.freq);

        // Premium leg uses CONTRACTUAL spread; protection leg uses calibrated h
        ...
        // charts
        var ts   = CDS.termStructure(p.marketSpreadBps, p.recoveryRate, p.riskFreeRate, p.freq);
        var surv = CDS.survivalCurve(h, p.maturity);
        var sens = CDS.recoverySensitivity(p.marketSpreadBps, p.recoveryRate, p.riskFreeRate, p.maturity, p.freq);
